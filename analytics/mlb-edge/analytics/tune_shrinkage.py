...
def main() -> None:
    ...
    if best_spread[0] < 200:
        print(f"[!] UNDERCONFIDENT: Try reducing shrinkage to {best_spread[0]:.0f}")
        print(f"    This will make predictions spread more like actual outcomes")
    elif best_spread[0] > 200:
        print(f"[!] OVERCONFIDENT: Try increasing shrinkage to {best_spread[0]:.0f}")
        print(f"    This will compress extreme predictions toward the mean")
    else:
        print(f"[OK] Current shrinkage (200) is near optimal for spread calibration")
